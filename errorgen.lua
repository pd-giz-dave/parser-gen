...
local function rightleaf(t)
  ...
  elseif action == "invert" then
    return addnot(rightleaf(op1))
  elseif action == "range" or action == "class" or action == "classset" then
    return {"_anychar"}
  else
...
local function printexpect(op)
  ...
  if isfinal(op) then
    if op["t"] then
      return "'"..op["t"].."'"
    elseif op["tchar"] then
      return "'"..op["tchar"].."'"
    end
    return op["nt"] or op["func"] or op["name"] or op["s"] or op["num"]
  else
    local test = op.op1
    if not test then
      return op.action
...
local function tryadderror(op, after)
  ...
  if FOLLOW then
    ...
    if generate then
      ...
      return {action="^LABEL",op1=op,op2={name=lab}}
    end
  end
  return op
end
